...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, len_padded = nn.utils.rnn.pad_packed_sequence(enc_hiddens, batch_first= True)

        last_hidden = torch.cat((last_hidden[0], last_hidden[1]), dim = 1)
        init_decoder_hidden = self.h_projection(last_hidden)                        #W_h

        last_cell = torch.cat((last_cell[0], last_cell[1]), dim = 1)
        init_decoder_cell = self.c_projection(last_cell)                            #W_c
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for i in torch.split(Y, 1, dim = 0):
            Yt = i.squeeze(dim=0)
            Ybar_t = torch.cat((Yt, o_prev), dim = 1)
            dec_state, ot, et = self.step(Ybar_t= Ybar_t, 
                                          dec_state= dec_state, 
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        (dec_hidden, dec_cell) = self.decoder(Ybar_t, hx = dec_state)
